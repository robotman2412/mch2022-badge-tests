...
void name_tag() {
	...
	pax_vec1_t lname_size = pax_text_size(font, font->default_size, lname);
	...
	while (1) {
		...
		pax_draw_text(&buf, color, font, name_scale,         0,       name_y,  name);
		pax_draw_text(&buf, color, font, font->default_size, lname_x, lname_y, lname);
